...
const materiData = {
  ...
  javascript:
    "JavaScript adalah bahasa pemrograman yang membuat web menjadi interaktif. Pelajari konsep dasar seperti variabel, fungsi, dan tipe data, hingga konsep lanjutan seperti closure, hoisting, asinkronus (Promises & async/await), dan manipulasi DOM.",
  react:
    "React adalah library UI paling populer saat ini. Fokus pada komponen fungsional, Hooks (useState, useEffect), state management tingkat lanjut, dan integrasi dengan ekosistem modern seperti Next.js.",
  tailwind:
    "Tailwind adalah utility-first CSS framework. Alih-alih menulis CSS terpisah, Anda menyusun class langsung di HTML untuk membangun antarmuka kustom secara cepat dan konsisten.",
  ...
  vitest:
    "Vitest adalah framework testing modern yang dirancang khusus untuk bekerja mulus dengan Vite. Cepat, mendukung TypeScript out-of-the-box, dan memiliki API yang sangat mirip dengan Jest.",
  prompt_eng:
    "Keahlian menyusun instruksi yang spesifik dan berkonteks tinggi agar AI (seperti Claude atau GPT) dapat menghasilkan kode yang lebih akurat, aman, dan sesuai dengan standar arsitektur proyek Anda.",
  biome:
    "Biome adalah toolchain modern untuk proyek web yang menggabungkan linter dan formatter dalam satu alat yang ditulis dengan Rust, sehingga performanya sangat cepat dibandingkan kombinasi ESLint dan Prettier.",
  owasp:
    "Mempelajari top 10 kerentanan keamanan web menurut OWASP (seperti XSS, CSRF, Injection). Developer frontend wajib memahami ini untuk mencegah celah keamanan sejak di sisi klien.",
  testing:
    "Testing sangat penting untuk memastikan kode berfungsi dengan baik di skala produksi. Kita menggunakan berbagai pendekatan: Unit Testing (menguji per fungsi kecil), Integration Testing (menguji antar komponen), dan E2E Testing (menguji alur pengguna seolah-olah oleh manusia di browser sesungguhnya).",
  bundlers:
    "Module bundlers mengumpulkan dan menyatukan semua file JavaScript, CSS, serta aset gambar menjadi satu kesatuan (bundle) sehingga browser dapat memuat aplikasi Anda secara efisien di environment produksi.",
  linters:
    "Linters & Formatters secara otomatis memeriksa kesalahan pada kode, memaksakan gaya penulisan yang konsisten antar tim, dan membantu mendeteksi bug (seperti variabel tak terpakai) sebelum kode dijalankan.",
  web_security:
    "Aspek keamanan tidak boleh diabaikan. Ini meliputi pemahaman CORS (berbagi sumber daya antar domain), HTTPS, CSP (untuk mencegah Cross-Site Scripting), dan berbagai celah keamanan yang rentan terjadi di sisi klien.",
  web_apis:
    "Web API menghubungkan bahasa pemrograman JavaScript dengan fungsionalitas device / browser Anda. API seperti Geolocation, Canvas, Web Storage, atau Web Sockets memungkinkan pembuatan aplikasi web tingkat lanjut (PWA).",
};
...
function openPanel(id, title, category, desc) {
  const actualDesc =
    ...
    (desc && desc !== "undefined"
      ? desc
      : `Pelajari materi <strong>${title}</strong> secara mendalam. Modul ini adalah fondasi penting yang akan terus Anda gunakan dalam praktik industri sehari-hari.`);

  const resources = resourcesDB[id] || resourcesDB.default;

  const freeResourcesHtml = resources
    .map(
      (res) => `
